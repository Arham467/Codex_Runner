...
import { GameObject, ObjectType, LANE_WIDTH, SPAWN_DISTANCE, REMOVE_DISTANCE, GameStatus, LETTER_COLORS, LEVEL_TARGETS } from '../../types';
...
const ParticleSystem: React.FC = () => {
    ...
    useFrame((state, delta) => {
        ...
        if (status === GameStatus.PAUSED || status === GameStatus.CREATOR) return;
...
export const LevelManager: React.FC = () => {
  ...
  useFrame((state, delta) => {
    if (status !== GameStatus.PLAYING || status === GameStatus.PAUSED || status === GameStatus.CREATOR) return;
         ...
         const isLetterDue = distanceTraveled.current >= nextLetterDistance.current;
          ...
          if (isLetterDue) {
             ...
             if (availableIndices.length > 0) {
                 ...
                 const color = LETTER_COLORS[val] || '#ffffff';
